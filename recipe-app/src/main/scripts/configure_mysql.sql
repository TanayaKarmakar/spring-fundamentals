...
CREATE USER 'app_dev_user'@'localhost' IDENTIFIED BY 'hacking';
CREATE USER 'app_prod_user'@'localhost' IDENTIFIED BY 'hacking';

#for Docker or remote server
CREATE USER 'app_dev_user'@'%' IDENTIFIED BY 'hacking';
CREATE USER 'app_prod_user'@'%' IDENTIFIED BY 'hacking';
...
GRANT DELETE ON app_prod.* to 'app_prod_user'@'localhost';
